...
open class OffensiveSpellsController : UIViewController {
    open static let STORYBOARD_ID : String = "OffensiveSpellsController"
    open static let ADVIEW_CALLBACK_IDENTIFIER : String = "OFFENSIVE_SPELLS_CALLBACK_IDENTIFIER"
    
    open class func newInstance() -> OffensiveSpellsController {
        return (UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: STORYBOARD_ID) as! OffensiveSpellsController)
    }
    ...
    internal var _targetProvince : Province!
    ...
    internal func _updateView() {
        ...
        for spell : Spell in _session.getAvailableSpells(SpellType.offensive) {
            let spellDuration : Int = _targetProvince.getSpellDuration(spell.getName())
    ...
    internal func _downloadData() {
        ...
        _session.downloadProvinceIntel(province: _targetProvince, callback: {
        ...
        _session.downloadAvailableSpells({
            (availableSpellsResponse : SessionResponse) -> Void in
                ...
                self._session.downloadActiveSpells(province: self._targetProvince, callback: {
    ...
    internal func _castSpell(spell: Spell, province : Province) {
        let loadingDialog : LoadingDialog = LoadingDialog.newInstance()
        loadingDialog.show(self.view)
        
        _session.castSpell(spell: spell, province: province, callback: {
    ...
    open func setTargetProvince(_ province: Province) {
    ...
    open override func viewDidLoad() {
        ...
        _spellsAdapter.setCellFactory(SpellCellFactory(province: _targetProvince))
        ...
        _spellsAdapter.setOnItemClickedCallback({
            (castableSpell: CastableSpell) -> Void in
                ...
                dialog.setOnSubmitCallback({
                    () -> Void in
                        self._castSpell(spell: castableSpell.getSpell(), province: self._targetProvince)
                })
